...
| `SWIMMERS_TMUX_SOCKET_NAME` | `(unset)` | Route tmux operations through `tmux -L <name>` for an isolated tmux server. Mutually exclusive with `SWIMMERS_TMUX_SOCKET_PATH`. |
| `SWIMMERS_TMUX_SOCKET_PATH` | `(unset)` | Route tmux operations through `tmux -S <path>` for an explicit tmux socket. Mutually exclusive with `SWIMMERS_TMUX_SOCKET_NAME`. |
...
For critical sessions that should survive a default tmux-server crash, start a
separate tmux namespace such as `tmux -L tiktok new-session -d -s tiktok` and
launch Swimmers with `SWIMMERS_TMUX_SOCKET_NAME=tiktok`. See
[`docs/tmux-crash-evidence.md`](docs/tmux-crash-evidence.md) for the incident
collector and isolated-socket runbook.
