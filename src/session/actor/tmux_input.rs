...
use super::run_bounded_tmux_command_for_target;
...
pub(super) async fn send_tmux_input_chunks(
    ...
    let _ = send_tmux_keys(&target, tmux_target, &["-X", "cancel"]).await;
    let mut delivered_chunks = 0;
    for chunk in chunks {
        let result = match chunk {
            TmuxInputChunk::Literal(text) => {
                send_tmux_keys(&target, tmux_target, &["-l", text]).await
            }
            TmuxInputChunk::Enter => send_tmux_keys(&target, tmux_target, &["Enter"]).await,
        };
        match result {
...
pub(super) async fn send_tmux_submit_line(
    tmux_name: &str,
    tmux_target: &TmuxTarget,
    text: &str,
) -> anyhow::Result<()> {
    ...
    set_tmux_buffer(tmux_target, &buffer_name, text).await?;
    paste_tmux_buffer(&target, tmux_target, &buffer_name).await?;
    send_tmux_keys(&target, tmux_target, &["Enter"]).await?;
